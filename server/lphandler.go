...
func (sess *Session) readOnce(req *http.Request) {
	if raw, err := ioutil.ReadAll(req.Body); err == nil {
		if msg := sess.dispatch(raw); msg != nil {
			// Write response packet back to channel
...
// Handle long poll connections when WebSocket is not available
...
func serveLongPoll(wrt http.ResponseWriter, req *http.Request) {
	...
	if sid == "" {
		sess := sessionStore.Create(wrt, appid)
		...
		enc.Encode(
			&ServerComMessage{Ctrl: &MsgServerCtrl{
				...
				Params: map[string]interface{}{"sid": sess.sid, "protocol": VERSION}}})

		// Any payload is ignored
		return
	}

	sess := sessionStore.Get(sid)
	if sess == nil {
